...
| `opensabre-starter-boot` | 应用启动基础能力，不传递 Servlet/OpenAPI UI。 | 固化基础配置与审计接入说明。 |
| `opensabre-starter-cache` | 缓存相关自动配置；可用 `opensabre.cache.enabled=false` 关闭。 | 补齐失效、序列化和测试规范。 |
...
| `opensabre-starter-persistence` | 持久化能力；MVC 异常映射仅在 Servlet 应用装配。 | 沉淀事务、迁移与审计约定。 |
| `opensabre-starter-eda` | 事件驱动架构能力；见模块内 README。 | 固化事件契约、幂等与可观测性。 |
| `opensabre-starter-governance` | 治理与审计相关能力。 | 补齐限流、审计、容错集成说明。 |
| `opensabre-starter-webmvc` | Servlet Web 应用能力，包含 OpenAPI/Knife4j；见模块内 README。 | 明确 MVC 扩展点与异常契约。 |
